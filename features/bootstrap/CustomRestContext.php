...
class CustomRestContext extends RestContext
{
    ...
    /**
     * @When After authentication on url :urlLogin with method :methodLogin as username :username and password :password, I send a :methodAddUser request to :urlAddUser with body:
     * @param $urlLogin
     * @param $methodLogin
     * @param $username
     * @param $password
     ...
     * @return mixed
     */
    public function afterAuthenticationOnUrlWithMethodAsUsernameAndPasswordISendARequestToWithBody($urlLogin, $methodLogin, $username, $password, $methodAddUser, $urlAddUser, PyStringNode $string)
    {
        $requestLogin = $this->request->send(
            $methodLogin,
